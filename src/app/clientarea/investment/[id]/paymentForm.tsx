...
export default function PaymentForm({ accountInfo, user, dbwallet }: any) {
  ...
  const checkinvestmentamout = (e: { target: { value: any } }) => {
    ...
    if (
      Number(accountInfo.minimum) > value ||
      value > Number(accountInfo.maximum)
    ) {
      setTooMuch(true);
    } else {
      setTooMuch(false);
  ...
  return (
    <div className="grid gap-5">
      ...
      <div className="grid md:grid-cols-5 md:gap-5 w-full p-4">
        <div className="font-bold pb-5 md:pb-0">Investment Details</div>
        <div className="col-span-4 w-full  bg-gray-100 rounded pt-10">
          ...
          <div className="w-full flex justify-between py-4 border-b border-gray-200 px-5">
            <div className="text-gray-600">Maximum Investment</div>
          ...
          <div className="w-full flex justify-between py-4 border-b border-gray-200 px-5">
            <div className="text-gray-600">ROI Payout</div>
            <div className="font-bold text-gray-800">{accountInfo.payout}</div>
          </div>

          <div className="w-full flex justify-between py-4 border-b border-gray-200 px-5">
            <div className="text-gray-600">Minimum Investment</div>
            <div className="font-bold text-gray-800">{accountInfo.minimum}</div>
          </div>

          <div className="w-full flex justify-between py-4 bg-blue-700 px-5 text-white rounded-b">
            <div>Returns On Investment</div>
            <div className="font-bold">{accountInfo.ROI}%</div>
          </div>
        </div>
      </div>

      <div className="grid md:grid-cols-5 md:gap-5 w-full p-4">
        <div className="font-bold pb-5 md:pb-0">Enter Amount</div>
        <div className="col-span-4 w-full  bg-gray-100 rounded py-10">
          <div className="flex justify-center w-full">
            <input
              className={`h-12 ${
                tooMuch ? "outline-1 outline-red-900" : "outline-none"
              }rounded-md pl-5 text-gray-600 mx-auto w-11/12 md:w-11/12`}
              type="number"
              onChange={checkinvestmentamout}
            />
          </div>
          <div
      ...
      <div className="grid md:grid-cols-5 md:gap-5 w-full p-4">
        ...
        {paymentMethord === "crypto" ? (
          <Button onClick={handleOpen} disabled={tooMuch}>
            Pay Now
          </Button>
        ) : (
      ...
      >
        ...
        >
          ...
          <div className="col-span-4">
            ...
            <div className="mt-10 text-center w-full">{wallet[0].address}</div>
          </div>
